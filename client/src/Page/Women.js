  ...
  return (
    <div className="Home"> {/* Main container for the Men page */}
      ...
      {products.map((product) => {
        if (product.category === `women's clothing`) { // Check if the product belongs to men's clothing category
          return <Product key={product.id} product={product} />;
        } else {
          return null; // Return null if the product doesn't belong to men's clothing category
        }
      })}
    </div>
  );
